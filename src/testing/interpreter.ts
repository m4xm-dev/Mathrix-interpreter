...
class Interpreter {
    ...
    private static readonly makeNum = (value: number): LiteToken => ({ name: "num", image: String(parseFloat(value.toFixed(10))) });


    private static readonly lCurly: LiteToken = { name: "lCurly", image: "{" };
    private static readonly rCurly: LiteToken = { name: "rCurly", image: "}" };


    private static readonly projectSteps = (
        innerSteps: LiteToken[][],
        left: LiteToken[],
        right: LiteToken[]
    ): LiteToken[][] =>
        innerSteps.map(step => left.concat(Interpreter.lCurly, step, Interpreter.rCurly, right));
    ...
    private eatBraceValue(left: LiteToken[]): BraceResult {
        ...
        const projected = Interpreter.projectSteps(innerSteps, left, right);
    ...
    private runLevels(t: LiteToken[], sink?: LiteToken[][]): LiteToken[] {
        ...
        const levelsLen = Interpreter.LEVELS.length;
        let current = t;

        for (this.levelIndex = 0; this.levelIndex < levelsLen;) {
            ...
            if (!reduced && level.has(name as any)) {
                switch (name) {
                    ...
                    case "exponentiation": {
                        ...
                        const exp = this.eatBraceValue(out.concat(baseTok, opTok));
                        out.push(Interpreter.makeNum(Math.pow(parseFloat(baseTok.image), exp.Result)));
                        break;
                    }

                    case "fraction": {
                        const fnTok = this.eat("fraction");
                        const num = this.eatBraceValue(out.concat(fnTok));
                        const den = this.eatBraceValue(out.concat(fnTok, Interpreter.lCurly, num.List, Interpreter.rCurly));
                        out.push(Interpreter.makeNum(Interpreter.safediv(num.Result, den.Result)));
                        break;
                    }

                    case "radication": {
                        const fnTok = this.eat("radication");
                        const val = this.eatBraceValue(out.concat(fnTok));
                        out.push(Interpreter.makeNum(Interpreter.safesqrt(val.Result)));
                        break;
                    }

                    case "trigonometry": {
                        const fnTok = this.next();
                        const arg = this.eatBraceValue(out.concat(fnTok));
                        out.push(Interpreter.makeNum(Interpreter.safetrig(fnTok.image, arg.Result)));
                        break;
                    }
                    ...
                    case "sLevel": {
                        ...
                        out.push(Interpreter.makeNum(op === "*" ? left * right : Interpreter.safediv(left, right)));
                        break;
                    }
                    ...
                    case "fLevel": {
                        ...
                        const op = this.next().image;
                        const right = parseFloat(this.eat("num").image);
                        out.push(Interpreter.makeNum(op === "+" ? left + right : left - right));
                        break;
                    }
                }
                reduced = true;
